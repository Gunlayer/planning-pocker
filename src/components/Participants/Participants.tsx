...
const Participants: FC<ParticipantsProps> = ({ participants, reveal, self }) => {
  return (
      ...
      {participants.map(({ id, name, card }) => {
        ...
        return (
          <motion.div
            key={id}
            layout
            className={`flex flex-col items-center p-4 rounded-2xl ${isSelf ? 'bg-white dark:bg-gray-700' : 'bg-white-100 dark:bg-gray-800'} shadow`}
          >
            <span className='font-medium mb-2 truncate max-w-[8rem]'>
              {isSelf ? `me(${name})` : name}
            </span>
            <motion.div
              initial={false}
